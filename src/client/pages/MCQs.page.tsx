...
import { mockTestQuestions } from "./questions";
...
export default function MultipleChoiceQuiz() {
  ...
  return (
    <div className="min-h-screen flex bg-white text-black pt-8">
      <div className="w-1/3 bg-green-200 p-6 flex flex-col justify-between">
        <div>
          ...
          <div className="mb-6">
            ...
            <p className="text-lg">Total Questions: {mockTestQuestions.length}</p>
          </div>
          <div className="grid grid-cols-4 gap-2 mb-4">
            {mockTestQuestions.map((_, index) => (
              <button
                key={index}
                className={`w-12 h-12 flex items-center justify-center rounded-lg text-white ${getQuestionStatus(index) === "attempted"
                  ? "bg-green-600"
      ...
      <div className="w-2/3 p-6 bg-gray-100 flex flex-col relative">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold">
            {mockTestQuestions[currentQuestionIndex].question}
          </h2>
        </div>
        <div className="space-y-2 mb-6">
          {mockTestQuestions[currentQuestionIndex].options.map((option, index) => (
            <button
              key={index}
              className={`w-full px-4 py-2 text-left rounded-lg ${selectedOptions[currentQuestionIndex] === option
                ? correctAnswer(
                  option,
                  mockTestQuestions[currentQuestionIndex].answer
                )
                : "bg-gray-300"
                } hover:bg-gray-400`}
        ...
        <div className="absolute bottom-4 right-4 flex space-x-4">
          ...
          <button
            className={`px-4 py-2 rounded-lg ${selectedOptions[currentQuestionIndex] ||
              skippedQuestions[currentQuestionIndex]
          ...
          <div className="mt-6">
            ...
            <ul>
              {mockTestQuestions.map((question, index) => (
                <li
                  key={index}
                  className={`mt-2 ${getQuestionStatus(index) === "attempted"
                    ? "text-green-600"
                    : getQuestionStatus(index) === "skipped"
                      ? "text-yellow-600"
                      : "text-gray-600"
